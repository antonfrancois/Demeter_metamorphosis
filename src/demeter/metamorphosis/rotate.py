...
class RigidMetamorphosis_integrator(Geodesic_integrator):
    ...
    def _step_old(self):
        ...
        id_rot = tb.matrix_time_grid(self.id_grid, eye + self.d_rot / self.n_step)
    ...
    def _compute_step_affine(self,
        ...
        d_affine = momentum_A @ A_mat.T   # d_affine = M
        ...
        if compute_field:
            ...
            inv_affine_grid = tb.matrix_time_grid(self.id_grid, inv_A_mat) - translation
        ...
        if compute_field:
            affine_field = affine_grid + tb.im2grid(field)/ self.n_step
            # affine_field = tb.compose_fields(tb.im2grid(field), affine_grid)  # TODO: Choisir la bonne chose.
        else:
            affine_field = affine_grid
        invA_affine_field = tb.matrix_time_grid(affine_field, inv_A_mat)
        # if torch.allclose(A_mat, torch.tensor([[1., 0.],[0., 1.]], device='cuda:0')):
        ...
        # tb.gridDef_plot_2d(self.id_grid.detach().cpu(), step = 10, ax = ax[0])
        # tb.gridDef_plot_2d(affine_field.detach().cpu(), step = 10, ax = ax[1])
        # tb.gridDef_plot_2d(invA_affine_field.detach().cpu(), step = 10, ax = ax[2])
        # ax[3].imshow(image.detach().cpu()[0,0])
        # plt.show()
        if self._get_rho_() != 1:
        ...
        if compute_field:
            momentum_I = self._compute_div_momentum_semiLagrangian_(
                ...
                momentum_I,
                cst=-sqrt(self.rho),
                field=sqrt(self.rho) * tb.im2grid(field)
            )
    ...
    def step_decoupled_rigidity(self, momentum_I, momentum_R, momentum_T, momentum_S, image, rot_mat, translation,
        ...
        return momentum_I, momentum_R, momentum_T, momentum_S, image, field, residuals, rot_mat, translation, scale
    ...
    def step(self, image, momentum_I, momentum_R, momentum_T, momentum_S, momentum_A, rot_mat, translation, scale):
        ...
        if momentum_A is not None:
            ...
            momentum_I, momentum_R, momentum_T, momentum_S, image, field, residuals, rot_mat, translation, scale = \
                self.step_decoupled_rigidity(momentum_I, momentum_R, momentum_T, momentum_S,
                                             image, rot_mat, translation, scale)
