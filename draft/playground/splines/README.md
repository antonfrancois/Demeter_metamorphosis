...
- **Numerical** contains a Cold/Warm spline initialization choice, the
  ...
  LBFGS learning rate, and the compute-device selector. Cold shows one spline
  parameter column. Warm splits the panel into
  ...
  available. Registration defaults to 10 optimization iterations and a `1.0`
  learning rate for both models. Rates at or below
...
Sobolev operator parameters, then transfers its momentum with zero
acceleration, jerk, and control fields. The spline stage always uses
source-cometric temporal block coordinates; saved and displayed shooting
fields remain in physical coordinates.
...
photometric-only image is currently selected. It can also plot global
optimization losses from the last registration.
